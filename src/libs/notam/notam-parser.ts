...
import {dateUTCFormat} from './utils/date-parser'
...
export const parserNotam = (text: string): INotam => {
    return {
        ...
        G: selectFiled(NOTAM_FIELD.G, text).replace(/\n|\)/g, ''),
...
export const createModel = (notam: INotam): IModelNotam => {
    ...
    return {
        ...
        area: A.trim(),
        schedule: {
            str: `${dateUTCFormat(dateStart, 'DD.MM.YY')}-${dateUTCFormat(dateEnd, 'DD.MM.YY')}, ${D}`,
            //str: `${dateUTCFormat(dateStart, 'DD.MM.YY HH:mm')}-${dateUTCFormat(dateEnd, 'DD.MM.YY HH:mm')}, ${D}`,
